/*
Special thanks to CodingJellyfish
*/
#include <cstdio>
#include <cstring>
#include <cstdlib>
...
#define SORTBYTE(TYPE,FR,TO,LEN,BIT) {\
    ...
    for (unsigned int *it = buc; it != buc + 255; it++)\
        it[1] += it[0];\
    for (TYPE *it = (FR) + LEN; it != (FR); it--)\
        (TO)[--buc[(it[-1] >> BIT) & 255]] = it[-1];\
}

int a[N];
int b[N];
void Radix_sort(int n) {
    SORTBYTE(int,a + 1,b + 1,n,0);
    SORTBYTE(int,b + 1,a + 1,n,8);
    SORTBYTE(int,a + 1,b + 1,n,16);
    SORTBYTE(int,b + 1,a + 1,n,24);
}
